...
public class PairsWithDiffK {
    public static int getPairsWithDifferenceK(int arr[], int k) {
        ...
        for (int i : arr) { // only iterate once... best approach
            ...
            int t1 = i - k, t2 = i + k;
            if (map.containsKey(t1) && k != 0)
                result += map.get(t1); // watch hint video last approach
            if (map.containsKey(t2) && k != 0)
                result += map.get(t2); // watch hint video last approach
        }
        int res2 = 0;
        if (k == 0) {
            for (Integer i : map.keySet())
                res2 += map.get(i) * (map.get(i) - 1) / 2; // nC2 = n(n-1)/2
            return res2;
        }
        return result;
    }
